...
def load_csv(input_directory, input_file_name):
    ...
    try:
        ...
        print(f"Matrix loaded successfully from {file_path}. Shape: {matrix.shape}")  
        # CHECK SHAPE
        print(matrix.shape)
        return matrix
...
def threshold_matrix(r_matrix, percentile):
    ...
    print(thresholded_matrix.shape)
...
def compute_distance_matrix(thresholded_matrix):
    ...
    print(zerolongmat_th_dis.shape)
...
def save_matrices(thresholded_matrix, distance_matrix, output_directory):
    """
