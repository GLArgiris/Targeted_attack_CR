...
def save_matrices(thresholded_matrix, distance_matrix, output_directory, output_file_name_prefix):
    ...
    Parameters:
        ...
        output_file_name_prefix (str): Prefix for the output file names.
    """
    os.makedirs(output_directory, exist_ok=True)
    np.save(os.path.join(output_directory, f"{output_file_name_prefix}_thresholded_matrix.npy"), thresholded_matrix)
    np.save(os.path.join(output_directory, f"{output_file_name_prefix}_distance_matrix.npy"), distance_matrix)
    print(f"Matrices saved to {output_directory} with prefix {output_file_name_prefix}")

def preprocess_data(input_directory, input_file_name, percentile, output_directory, output_file_name_prefix):
    """
    ...
    save_matrices(thresholded_matrix, distance_matrix, output_directory, output_file_name_prefix)
...
def main():
    ...
    """
    if len(sys.argv) != 6:
        print("Usage: python preprocess_data.py <input_directory> <input_file_name> <percentile_threshold> <output_directory> <output_file_name_prefix>")
        sys.exit(1)
    ...
    output_file_name_prefix = sys.argv[5]  # Prefix for the output files (.npy)

    preprocess_data(input_directory, input_file_name, percentile_threshold, output_directory, output_file_name_prefix)
